...
module Automation.lib.generateBetaIndHitPath where
...
getβrecRtypePath Rpnt indTyp pathInd elim baseRec indLs points paths d1 0 ref (pi (arg (arg-info vis rel) t1) (abs s t2)) =
  do t2' ← (getβrecRtypePath Rpnt indTyp pathInd elim baseRec indLs points paths d1 0 ref t2)
     pure t2'
getβrecRtypePath Rpnt indTyp pathInd elim baseRec indLs points paths d1 (suc d) ref (pi (arg (arg-info vis rel) t1) (abs s t2)) =
  do t2' ← (getβrecRtypePath Rpnt indTyp pathInd elim baseRec indLs points paths d1 d (suc ref) t2)
     pure (pi (arg (arg-info hidden rel) t1) (abs s t2'))
getβrecRtypePath Rpnt indTyp pathInd elim baseRec indLs points paths d1 d ref (agda-sort Level) =
  do lcons ← (getLength points)
     ...
     mapPath ← (βIndMapPath Rpnt pathInd elim parsPath consPath [] indTyp index pathTyp')
...
getβrecRtypePath Rpnt indType pathInd elim baseRec indLs points paths d1 d ref x = pure unknown
...
generateβRec' (arg i f) R pathInd elim baseElim indLs indType d points paths = 
  do RTy ← (getType indType)
     funTypePath ← (getβrecRtypePath R indType pathInd elim baseElim indLs points paths d d zero RTy)
     -- (debugPrint "tc.sample.debug" 20 (strErr "issue : beta path ---> " ∷ termErr funTypePath ∷ []))
     (declarePostulate (arg i f) funTypePath)

generateβRec'' : List (Arg Name) → (Lpoints : List Name) → (pathInd : Nat) → (elim : Name) → (baseElim : Name) →
  (indexList : List Nat) → (indType : Name) → (params : Nat) → (points : List Name) → (paths : List Name) → TC ⊤
generateβRec'' (x ∷ xs) (p ∷ ps) (suc pathInd) elim baseElim indLs indType d points paths =
  do (generateβRec' x p pathInd elim baseElim indLs indType d points paths)
     (generateβRec'' xs ps pathInd elim baseElim indLs indType d points paths)
generateβRec'' [] p pathInd elim baseElim indLs indType d points paths = pure tt
generateβRec'' n p pathInd elim baseElim indLs indType d points paths = pure tt

generateβIndHitPath : (elim : Name) → (comp : List (Arg Name)) →
  (indType : Name) → (baseElim : Name) → (param : Nat) → (points : List Name) → (paths : List Name) → TC ⊤
generateβIndHitPath elim comp indType baseElim d points paths =
  do exp ← (getExprRef2 indType d points)
     argL ← (getLength comp)
     (generateβRec'' comp paths argL elim baseElim exp indType d points paths)
