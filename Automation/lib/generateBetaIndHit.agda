...
module Automation.lib.generateBetaIndHit where
...
getβrecRtypePath : (Rpoint : Name) → (indTyp : Name) → (pathInd : Nat) → (elim : Name) → (baseElim : Name) → 
  (indexList : List Nat) → (indL : Nat) → (points : List Name) → (paths : List Name) → (param : Nat) → (pars : Nat) → (ref : Nat) → (RTy : Type) → TC Type
getβrecRtypePath Rpnt indTyp pathInd elim baseRec indLs indL points paths d1 0 ref (pi (arg (arg-info vis rel) t1) (abs s t2)) =
  do t2' ← (getβrecRtypePath Rpnt indTyp pathInd elim baseRec indLs indL points paths d1 0 ref t2)
     pure t2'
getβrecRtypePath Rpnt indTyp pathInd elim baseRec indLs indL points paths d1 (suc d) ref (pi (arg (arg-info vis rel) t1) (abs s t2)) =
  do t2' ← (getβrecRtypePath Rpnt indTyp pathInd elim baseRec indLs indL points paths d1 d (suc ref) t2)
     pure (pi (arg (arg-info hidden rel) t1) (abs s t2'))
getβrecRtypePath Rpnt indTyp pathInd elim baseRec indLs indL points paths d1 d ref (agda-sort Level) =
  do lcons ← (getLength points)
     ...
     RTyp ← (getType indTyp)
     RTyp' ← (rmPars d1 RTyp)
     mapPath ← (βrecMapPath Rpnt (pathInd + lpaths) elim d1 parsPath consPath [] indTyp indL indL pathTyp')
     -- (debugPrint "tc.sample.debug" 20 (strErr "issue : comes here in Induction *** ---> " ∷ termErr mapPath ∷ []))
     pathTyp ← (getβIndPaths baseRec d1 mapPath zero parsPnt consPnt indTyp points paths)
     funType ← (getMapConsTypeListInd indTyp d1 zero pathTyp pars indLs points points)
     CType ← (getCTypeInd indTyp pars' d RTyp')
     pure (pi (vArg CType) (abs "C" funType))
getβrecRtypePath Rpnt indType pathInd elim baseRec indLs indL points paths d1 d ref x = pure unknown
...
generateβRec' : Arg Name → (R : Name) → (pathInd : Nat) → (elim : Name) → (baseElim : Name) → 
  (indexList : List Nat) → (indL : Nat) → (indType : Name) → (param : Nat) → (points : List Name) → (paths : List Name) → TC ⊤
generateβRec' (arg i f) R pathInd elim baseElim indLs indL indType d points paths = 
  do RTy ← (getType indType)
     funTypePath ← (getβrecRtypePath R indType pathInd elim baseElim indLs indL points paths d d zero RTy)
...
generateβRec'' : List (Arg Name) → (Lpoints : List Name) → (indList : List Nat) → (pathInd : Nat) → (elim : Name) → (baseElim : Name) →
  (indexList : List Nat) → (indType : Name) → (params : Nat) → (points : List Name) → (paths : List Name) → TC ⊤
generateβRec'' (x ∷ xs) (p ∷ ps) (i ∷ is) (suc pathInd) elim baseElim indLs indType d points paths =
  do (generateβRec' x p pathInd elim baseElim indLs i indType d points paths)
     (generateβRec'' xs ps is pathInd elim baseElim indLs indType d points paths)
generateβRec'' [] p il pathInd elim baseElim indLs indType d points paths = pure tt
generateβRec'' n p il pathInd elim baseElim indLs indType d points paths = pure tt

generateElim : Arg Name → (indType : Name) → (baseElim : Name) → (param : Nat) → (indexList : List Nat) → (points : List Name) → (paths : List Name) → TC ⊤
generateElim (arg i f) indType baseElim d exp points paths = 
  do RTy ← (getType indType)
...
generateβIndHit : (elim : Arg Name) → (comp : List (Arg Name)) →
  (indType : Name) → (baseElim : Name) → (param : Nat) → (points : List Name) → (paths : List Name) → TC ⊤
generateβIndHit (arg i elim) comp indType baseElim d points paths =
  do exp ← (getExprRef2 indType d points)
     argL ← (getLength comp)
     (generateElim (arg i elim) indType baseElim d exp points paths)
